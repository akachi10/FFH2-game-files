...
##				if i == -1:
##					continue
##				if self.areaMap.areaMap[i] == biggestContinentID and not hm.isBelowSeaLevel(x,y): #this helps narrow the search
...
##					print "suitability on areaID=%(aid)d, size=%(s)d is %(r)f" % {"aid":area.getID(),"s":area.getNumTiles(),"r":aS.suitability}
...
##			areasPerBonus = (len(areaSuitabilityList)/numUniqueBonuses) + 1
	...
	def PlotCanHaveBonus(self,plot,eBonus,bIgnoreLatitude,bIgnoreArea):
		...
		if (not bIgnoreArea and bonusInfo.isOneArea() and not mc.spreadResources):
			areaID = plot.getArea()
			areaFound = False
...
##		print "baseCount=%(bc)d, numPossible=%(np)d, landTiles=%(lt)d, players=%(p)d" % {"bc":baseCount,"np":numPossible,"lt":landTiles,"p":players}
##		print ""
		return bonusCount
...
#		game.gc.getGame()
	...
	def getPlotPotentialValue(self,x,y,coastalCity):
		...
		if bonusInfo != None and (gc.getTechInfo(bonusInfo.getTechCityTrade()) == None or gc.getTechInfo(bonusInfo.getTechCityTrade()).getEra() <= game.getStartEra()) and (gc.getTechInfo(bonusInfo.getTechReveal()) == None or gc.getTechInfo(bonusInfo.getTechReveal()).getEra() <= game.getStartEra()): 
			if bonusInfo == None:
	...
	def ensureMinimumHills(self,x,y):
		...
		if hillsNeeded > 0:
			for plot in plotList:
				...
				featureInfo = gc.getFeatureInfo(plot.getFeatureType())
				bonusInfo = gc.getBonusInfo(plot.getBonusType(TeamTypes.NO_TEAM))
				if plot.getPlotType() != PlotTypes.PLOT_HILLS and plot.getArea() == gameMap.plot(x,y).getArea() and (featureInfo == None or not featureInfo.isRequiresFlatlands()) and (bonusInfo == None or not bonusInfo.isRequiresFlatlands()):
					plot.setPlotType(PlotTypes.PLOT_HILLS,True,True)
					hillsNeeded -= 1
					print "adding hill"
			if hillsNeeded > 0:
...
##					print "distance between n=%(n)d nx=%(nx)d,ny=%(ny)d and m=%(m)d mx=%(mx)d,my=%(my)d is %(d)d or %(nd)d" % {"n":n,"nx":nPlot.getX(),"ny":nPlot.getY(),"m":m,"mx":mPlot.getX(),"my":mPlot.getY(),"d":distance,"nd":newDistance}
...
##			print "Placing start at x=%(x)d, y=%(y)d nearestDistance to city is %(n)d" % {"x":distanceList[0].x,"y":distanceList[0].y,"n":distanceList[0].nearestStart}
...
##					print "n= %(n)3d, m = %(m)3d, p = %(p)3d, nDistance = %(nd)3d, pDistance = %(pd)3d" %{"n":n,"m":m,"p":p,"nd":nDistance,"pd":pDistance}
...
class StartPlot :
	...
	def copy(self):
		...
		return cp
	def __str__(self):
		linestring = "x=%(x)3d,y=%(y)3d,localValue=%(lv)6d,totalValue =%(tv)6d, nearestStart=%(ad)6d, coastalCity=%(cc)d" % {"x":self.x,"y":self.y,"lv":self.localValue,"tv":self.totalValue,"ad":self.nearestStart,"cc":self.isCoast()}
		return linestring
...
def getGridSize(argsList):
	grid_sizes = {
		...
		WorldSizeTypes.WORLDSIZE_HUGE:		(43,29)
	}
...
##	PrintRainMap(rm,rm.rainMap,False)
	for y in range(mc.height):
		for x in range(mc.width):
			...
			plot = mmap.plot(x,y)
			#forest and jungle
			if not plot.isWater() and sm.terrainMap[i] != mc.DESERT and not plot.isPeak():
			...
			elif sm.terrainMap[i] == mc.DESERT and sm.plotMap[i] != mc.HILLS and sm.plotMap[i] != mc.PEAK and not plot.isWater():
