...
#		print "[FracW] -- PlateTectonics:breakContinent()"
	...
	def compactifyContinent( self, continentID=None, chCompact=33, chSink=66, chMove=75 ):
		...
		if continentID == None:
			...
			coordList = [ ( x, y, mst.numWaterNeighbors(x, y) ) for x,y in cPlots if mst.numWaterNeighbors(x,y) < 4 ]
			if len( coordList ) > 0:
			...
			coordList = [ (x, y, mst.numWaterNeighbors(x, y) ) for x,y in aPlots if mst.numWaterNeighbors(x, y) >= 6 ]
			if len( coordList ) > 0:
				for x,y,n in coordList:
...
class OccStart:
	...
	def occHumanStart( self ):
		...
		humanList = [ playerID for playerID in range( gc.getMAX_CIV_PLAYERS() ) if gc.getPlayer( playerID ).isAlive() if gc.getPlayer( playerID ).isHuman() ]
		...
		occStartList = [ playerID for playerID in playerNeighborAreasDict.keys() if oceanID in playerNeighborAreasDict[ playerID ] ]
...
class MapOptions:
	...
	def writeConfig( self ):
		...
		# prepare map options
		self.userOptions = [ int(self.Shape), int(self.Resources), int(self.Water), int(self.Peaks), int(self.Terrain), int(self.Age), int(self.Coast), int(self.Theme), int(self.Teams) ]
