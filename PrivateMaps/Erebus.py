...
def GetImprovementPreferences():
	...
	pref = ImprovementPreference(GetInfoType("IMPROVEMENT_WODES_OAK"))
	pref.idealMoisture = .75
	pref.idealAltitude = .25
	pref.moistureWeight = 2.0
	pref.altitudeWeight = 1.0
	...
	pref.needHill = False
	pref.needFlat = False
	pref.favoredTerrain = GetInfoType("TERRAIN_PLAINS")
	impPreferenceList.append(pref)
			...
			for x in range(mapSize.MapWidth):
				...
				if plotMap.plotMap[i] == plotMap.PEAK:
					...
					for direction in range(1,9,1):
						xx,yy = self.getXYFromDirection(x,y,direction)
						ii = GetIndex(xx,yy)
						if plotMap.plotMap[ii] != plotMap.PEAK and plotMap.plotMap[ii] != plotMap.OCEAN:
...
def getGridSize(argsList):
	...
	grid_sizes = {
		...
		WorldSizeTypes.WORLDSIZE_HUGE:		(int(22 * scaler),int(22 * scaler))
