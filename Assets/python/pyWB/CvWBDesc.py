...
class CvTeamDesc:
	...
	def write(self, f, idx):
		...
		f.write("\tTeamID=%d, (%s)\n" %(idx, pTeam.getName().encode(fileencoding,'replace')))
		...
		for i in range(iNumTeams):
			if pTeam.getEspionagePointsAgainstTeam(i) > 0:
				f.write("\tEspionageTeam=%d, EspionageAmount=%d, (%s)\n" %(i, pTeam.getEspionagePointsAgainstTeam(i), gc.getTeam(i).getName().encode(fileencoding,'replace')))
	...
	## Platy Builder ##
		for i in range(iNumTeams):
			...
			if pTeam.isHasMet(i):
				f.write("\tContactWithTeam=%d, (%s)\n" %(i, gc.getTeam(i).getName().encode(fileencoding,'replace')))
	## Platy Builder ##
		# write warring teams
		for i in range(iNumTeams):
			if pTeam.isAtWar(i):
		...
		for i in range(iNumTeams):
			if pTeam.isVassal(i):
				...
				if iType == 1:
					f.write("\tVassalOfTeam=%d, Type=%s, (%s)\n" %(i, "FREE", gc.getTeam(i).getName().encode(fileencoding,'replace')))
				elif iType == 0:
					f.write("\tVassalOfTeam=%d, Type=%s, (%s)\n" %(i, "CAP", gc.getTeam(i).getName().encode(fileencoding,'replace')))
...
#Magister Stop
	...
	## Platy Builder ##
		...
		if pPlayer.getCivilizationType() != CivilizationTypes.NO_CIVILIZATION:
			f.write("\tLeaderName=%s\n" %(pPlayer.getNameKey().encode(fileencoding,'replace')))
			f.write("\tCivDesc=%s\n" %(pPlayer.getCivilizationDescriptionKey().encode(fileencoding,'replace')))
			f.write("\tCivShortDesc=%s\n" %(pPlayer.getCivilizationShortDescriptionKey().encode(fileencoding,'replace')))
			f.write("\tCivAdjective=%s\n" %(pPlayer.getCivilizationAdjectiveKey().encode(fileencoding,'replace')))
			f.write("\tFlagDecal=%s\n" %(pPlayer.getFlagDecal().encode(fileencoding,'replace')))
			f.write("\tWhiteFlag=%d\n" %(pPlayer.isWhiteFlag(),))
			...
			for i in range(iNumPlayers):
				if pPlayer.AI_getAttitudeExtra(i) != 0:
					f.write("\tAttitudePlayer=%d, AttitudeExtra=%d\n, (%s)" %(i, pPlayer.AI_getAttitudeExtra(i), gc.getPlayer(i).getName().encode(fileencoding,'replace')))
...
#Magister Stop
	...
	def write(self, f, unit, plot):
		...
		f.write("\tBeginUnit\n")
		f.write("\t\tUnitType=%s, UnitOwner=%d, (%s)\n" %(Info.getType(), unit.getOwner(), gc.getPlayer(unit.getOwner()).getName().encode(fileencoding,'replace')))
		if (len(unit.getNameNoDesc()) > 0):
			f.write("\t\tUnitName=%s\n" %(unit.getNameNoDesc().encode(fileencoding,'replace'),))
...
#Magister Stop
	...
	def write(self, f, plot):
		...
		f.write("\tBeginCity\n")
		f.write("\t\tCityOwner=%d, (%s)\n" %(city.getOwner(), gc.getPlayer(city.getOwner()).getName().encode(fileencoding,'replace')))
		f.write("\t\tCityName=%s\n" %(city.getName().encode(fileencoding,'replace'),))
		...
		for iPlayerLoop in range(iNumPlayers):
			...
			if (iPlayerCulture > 0):
				f.write("\t\tPlayer%dCulture=%d, (%s)\n" %(iPlayerLoop, iPlayerCulture, gc.getPlayer(iPlayerLoop).getName().encode(fileencoding,'replace')))
...
class CvSignDesc:
	...
	def write(self, f, sign):
		...
		if sign.getPlayerType() > -1:
			f.write("\tplayerType=%d, (%s)\n" %(sign.getPlayerType(), gc.getPlayer(sign.getPlayerType()).getName().encode(fileencoding,'replace')))
		f.write("\tcaption=%s\n" %(sign.getCaption().encode(fileencoding,'replace'),)) # lfgr 07/2019 encoding fix
		f.write("EndSign\n")
