...
#		self.iPermanentSnowChance = 30			#Chance a blizzard turns land plots into permanent snow
	...
	def doBlizzardTurn(self):
		...
		iGlacier = gc.getInfoTypeForString('TERRAIN_GLACIER')#Magister
		iWasteland = gc.getInfoTypeForString('TERRAIN_WASTELAND')#Magister
		iWhiteHand = gc.getInfoTypeForString('RELIGION_WHITE_HAND')#Magister
		...
		for i in range(CyMap().numPlots()):
			...
			if pPlot.getFeatureType() == iBlizzard:
				...
				#Kills a blizzard
				if pPlot.isOwned() and gc.getPlayer(pPlot.getOwner()).getStateReligion() == iWhiteHand:
			...
			#Creates a blizzard
			if pPlot.getTerrainType() in [iSnow, iGlacier]:#Magister
	...
	def moveBlizzard(self, pPlot, iDirection):
		iBlizzard = gc.getInfoTypeForString('FEATURE_BLIZZARD')
		iBB = gc.getInfoTypeForString('IMPROVEMENT_BADBS_BLIZZARD')#Magister
		...
		if self.canBlizzard(newPlot, False):
			...
			if pPlot.getRealImprovementType() == iBB:
				newPlot.setImprovementType(iBB)
...
#				return False
			...
			if bNew == True:
				...
				if not pPlot.isOwned():
					return False
				if gc.getPlayer(pPlot.getOwner()).getStateReligion() != iWhiteHand:
					return False
		...
		if not pPlot.isNone():
			...
			if not pPlot.isWater():
				if pPlot.getTempTerrainTimer() < iTurns: #Only convert existing temporary terrain if our timer will be longer than current timer
					if not pPlot.getTerrainType() in [iSnow, iGlacier]:
						if pPlot.getPlotCounter() < 50:
							...
							if pPlot.getTerrainType() == iWasteland or  pPlot.isHills():
								pPlot.setTempTerrainType(iGlacier, iTurns)
							else:
								pPlot.setTempTerrainType(iWasteland, iTurns)
