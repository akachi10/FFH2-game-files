...
bShowManaBar = 1
lManaTypes = [	'BONUS_MANA_AIR',
				'BONUS_MANA_BODY',
				'BONUS_MANA_CHAOS',
				'BONUS_MANA_CREATION',
				'BONUS_MANA_DEATH',
				'BONUS_MANA_DIMENSIONAL',
				'BONUS_MANA_EARTH',
				'BONUS_MANA_ENCHANTMENT',
				'BONUS_MANA_ENTROPY',
				'BONUS_MANA_FIRE',
				'BONUS_MANA_FORCE',
				'BONUS_MANA_ICE',
...
#FfH: End Modify
		...
		screen.addPanel( "TopCityPanelRight", u"", u"", True, False, xResolution/2, 70, yResolution/2-260, 60, PanelStyles.PANEL_STYLE_STANDARD )
		...
		screen.setImageButton("RawManaButton1", "Art/Interface/Screens/RawManaButton.dds", 0, 65, 20, 20, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.hide( "RawManaButton1" )
		screen.addPanel( "ManaToggleHelpTextPanel", u"", u"", True, True, 15, 475, 170, 30, PanelStyles.PANEL_STYLE_HUD_HELP )
		...
		screen.addMultilineText( "ManaToggleHelpText", szText, 15, 475, 167, 27, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
...
# CCV - Position of Scores - END
		...
		screen.setImageButton( "InterfaceHelpButton", ArtFileMgr.getInterfaceArtInfo("INTERFACE_GENERAL_CIVILOPEDIA_ICON").getPath(), 2, 2, 24, 24, WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_CIVILOPEDIA).getActionInfoIndex(), -1 )
		screen.hide( "InterfaceHelpButton" )

		screen.setImageButton( "MainMenuButton", ArtFileMgr.getInterfaceArtInfo("INTERFACE_GENERAL_MENU_ICON").getPath(), xResolution - 26, 2, 24, 24, WidgetTypes.WIDGET_MENU_ICON, -1, -1 )
...
# BUG - Bars on single line for higher resolution screens - end
		...
		# Holy City Overlay
		for i in range(0, gc.getNumReligionInfos()/3 ):
		...
		for i in range(0, gc.getNumReligionInfos()/3 ):
			xCoord = xResolution - 242 + (i * 30)
			yCoord = 66
			szName = "ReligionHolyCityDDS" + str(gc.getNumReligionInfos()/3+i)
			screen.addDDSGFC( szName, ArtFileMgr.getInterfaceArtInfo("INTERFACE_HOLYCITY_OVERLAY").getPath(), xCoord, yCoord, 24, 24, WidgetTypes.WIDGET_HELP_RELIGION_CITY, i, -1 )
			screen.hide(szName)
		...
		for i in range(0, gc.getNumReligionInfos()/3 ):
			xCoord = xResolution - 242 + (i * 30)
			yCoord = 90
			szName = "ReligionHolyCityDDS" + str(gc.getNumReligionInfos()*2/3+i)
			screen.addDDSGFC( szName, ArtFileMgr.getInterfaceArtInfo("INTERFACE_HOLYCITY_OVERLAY").getPath(), xCoord, yCoord, 24, 24, WidgetTypes.WIDGET_HELP_RELIGION_CITY, i, -1 )
			screen.hide(szName)
...
####magister start
##							iRace = pHeadSelectedUnit.getRace()
##							if iRace != -1:
##								szButton = gc.getUnitInfo(gc.getActionInfo(i).getCommandData()).getUnitStyleButton(iRace)
##								if szButton != -1:
##									szButton = gc.getActionInfo(i).getButton()
...
#Magister--the real gold rate should take into account the mercenary fees that are handled in python
					if pPlayer.getCivilizationType() != gc.getInfoTypeForString('CIVILIZATION_HIPPUS'):
						iMercenary = gc.getInfoTypeForString('PROMOTION_MERCENARY')
						(loopUnit, i) = pPlayer.firstUnit(False)
						while(loopUnit):
							if loopUnit.isHasPromotion(iMercenary):
								if not loopUnit.isDelayedDeath():
									iGoldRate -= (loopUnit.getLevel() + gc.getUnitInfo(loopUnit.getUnitType()).getTier())/2
							(loopUnit, i) = pPlayer.nextUnit(i, False)
#Magister
...
##					iGold = (gc.getPlayer(ePlayer).getGold() / (gc.getPlayer(ePlayer).getNumCities()
				...
				iNumVaults = gc.getPlayer(ePlayer).getBuildingClassCount(gc.getInfoTypeForString('BUILDINGCLASS_DWARVEN_VAULT'))
				if iNumVaults > 0:
					iGold = gc.getPlayer(ePlayer).getGold() / iNumVaults

					if iGold < 25:
						szText += " " + localText.getText("TXT_KEY_MISC_DWARVEN_VAULT_EMPTY", ())
					elif iGold < 75:
						szText += " " + localText.getText("TXT_KEY_MISC_DWARVEN_VAULT_LOW", ())
					elif iGold < 150:
						pass
					elif iGold < 250:
						szText += " " + localText.getText("TXT_KEY_MISC_DWARVEN_VAULT_STOCKED", ())
					elif iGold < 500:
						szText += " " + localText.getText("TXT_KEY_MISC_DWARVEN_VAULT_ABUNDANT", ())
					elif iGold < 1000:
						szText += " " + localText.getText("TXT_KEY_MISC_DWARVEN_VAULT_FULL", ())
					else:
						szText += " " + localText.getText("TXT_KEY_MISC_DWARVEN_VAULT_OVERFLOWING", ())
#FfH: End Add
				#MOVED AMOUNT OF GOLD TEXT							2/03/08					JOHNY SMITH
				# lfgr 04/2021: Changed widget type to show Khazad vault tooltip
				screen.setLabel( "GoldText", "Background", szText, CvUtil.FONT_LEFT_JUSTIFY, 30, 6, -0.3, FontTypes.GAME_FONT, WidgetTypes.WIDGET_PLAYER_GOLD, -1, -1 )
				#END													2/03/08					JOHNY SMITH
...
#FfH: End Add
		...
		screen.addPanel( "BonusPane0", u"", u"", True, False, xResolution - 244, 120, 57, yResolution - 520, PanelStyles.PANEL_STYLE_CITY_COLUMNL )
		screen.hide( "BonusPane0" )
		screen.addScrollPanel( "BonusBack0", u"", xResolution - 242, 120, 157, yResolution - 536, PanelStyles.PANEL_STYLE_EXTERNAL )
		screen.hide( "BonusBack0" )

		screen.addPanel( "BonusPane1", u"", u"", True, False, xResolution - 187, 120, 68, yResolution - 520, PanelStyles.PANEL_STYLE_CITY_COLUMNC )
		screen.hide( "BonusPane1" )
		screen.addScrollPanel( "BonusBack1", u"", xResolution - 191, 120, 184, yResolution - 536, PanelStyles.PANEL_STYLE_EXTERNAL )
		screen.hide( "BonusBack1" )

		screen.addPanel( "BonusPane2", u"", u"", True, False, xResolution - 119, 120, 107, yResolution - 520, PanelStyles.PANEL_STYLE_CITY_COLUMNR )
		screen.hide( "BonusPane2" )
		screen.addScrollPanel( "BonusBack2", u"", xResolution - 125, 120, 205, yResolution - 536, PanelStyles.PANEL_STYLE_EXTERNAL )
...
#FfH: End Add
				...
				for i in range(0,gc.getNumReligionInfos()/3):
					...
					if pHeadSelectedCity.isHasReligion(gc.getNumReligionInfos()/3+i):

#FfH: Added by Kael 11/03/2007
						if gc.getPlayer(gc.getGame().getActivePlayer()).canSeeReligion(gc.getNumReligionInfos()/3+i):
#FfH: End Add

							if pHeadSelectedCity.isHolyCityByType(gc.getNumReligionInfos()/3+i):
								szTempBuffer = u"%c" %(gc.getReligionInfo(gc.getNumReligionInfos()/3+i).getHolyCityChar())
								szName = "ReligionHolyCityDDS" + str(gc.getNumReligionInfos()/3+i)
								screen.show(szName)
							else:
								szTempBuffer = u"%c" %(gc.getReligionInfo(gc.getNumReligionInfos()/3+i).getChar())
							szBuffer += szTempBuffer

						j = 0
						for j in range(CommerceTypes.NUM_COMMERCE_TYPES):
							iCommerce = pHeadSelectedCity.getReligionCommerceByReligion(j, gc.getNumReligionInfos()/3+i)
						...
						iHappiness = pHeadSelectedCity.getReligionHappiness(gc.getNumReligionInfos()/3+i)
						...
						szButton = gc.getReligionInfo(gc.getNumReligionInfos()/3+i).getButton()
					...
					szName = "ReligionDDS" + str(gc.getNumReligionInfos()/3+i)
					screen.setImageButton( szName, szButton, xCoord, yCoord, 24, 24, WidgetTypes.WIDGET_HELP_RELIGION_CITY, gc.getNumReligionInfos()/3+i, -1 )
					screen.enable( szName, bEnable )
					screen.show(szName)
				...
				for i in range(0,gc.getNumReligionInfos()/3):
					...
					if pHeadSelectedCity.isHasReligion(gc.getNumReligionInfos()*2/3+i):

#FfH: Added by Kael 11/03/2007
						if gc.getPlayer(gc.getGame().getActivePlayer()).canSeeReligion(gc.getNumReligionInfos()*2/3+i):
#FfH: End Add

							if pHeadSelectedCity.isHolyCityByType(gc.getNumReligionInfos()*2/3+i):
								szTempBuffer = u"%c" %(gc.getReligionInfo(gc.getNumReligionInfos()*2/3+i).getHolyCityChar())
								szName = "ReligionHolyCityDDS" + str(gc.getNumReligionInfos()*2/3+i)
								screen.show(szName)
							else:
								szTempBuffer = u"%c" %(gc.getReligionInfo(gc.getNumReligionInfos()*2/3+i).getChar())
							szBuffer += szTempBuffer

						j = 0
						for j in range(CommerceTypes.NUM_COMMERCE_TYPES):
							iCommerce = pHeadSelectedCity.getReligionCommerceByReligion(j, gc.getNumReligionInfos()*2/3+i)
						...
						iHappiness = pHeadSelectedCity.getReligionHappiness(gc.getNumReligionInfos()*2/3+i)
						...
						szButton = gc.getReligionInfo(gc.getNumReligionInfos()*2/3+i).getButton()
					...
					szName = "ReligionDDS" + str(gc.getNumReligionInfos()*2/3+i)
					screen.setImageButton( szName, szButton, xCoord, yCoord, 24, 24, WidgetTypes.WIDGET_HELP_RELIGION_CITY, gc.getNumReligionInfos()*2/3+i, -1 )
					screen.enable( szName, bEnable )
					screen.show(szName)
...
#FfH: End Add
		...
		#FfH: End Add
												...
												lTeammates = cf.getTeammates(gc.getPlayer(gc.getGame().getActivePlayer()))
#FfH: Added by Kael 10/01/2008
##												if gc.getPlayer(gc.getGame().getActivePlayer()).isFullMember(gc.getInfoTypeForString('DIPLOVOTE_UNDERCOUNCIL')):
...
##														for pTeammate in lTeammates:
##															if pTeammate.getNumBuilding(gc.getInfoTypeForString('BUILDING_EYES_AND_EARS_NETWORK')) > 0:
##																bEspionageCanSeeResearch = True
##																break
...
#FfH: End Add
												...
												bCanSeeResearch = False



												if bEspionageCanSeeResearch:
													bCanSeeResearch = True
												if gc.getGame().isDebugMode():
													bCanSeeResearch = True
												if (gc.getPlayer(ePlayer).getTeam() == gc.getGame().getActiveTeam()) and (gc.getTeam(gc.getGame().getActiveTeam()).getNumMembers() > 1):
													bCanSeeResearch = True
												if gc.getTeam(gc.getPlayer(ePlayer).getTeam()).isVassal(gc.getGame().getActiveTeam()):
													bCanSeeResearch = True
...
##																bCanSeeResearch = True
##																break
##Maybe stolen visibility includes this
##												if CyGame().getBuildingClassCreatedCount(gc.getInfoTypeForString('BUILDINGCLASS_EYES_AND_EARS_NETWORK')) > 0:
##													for pTeammate in lTeammates:
##														if pTeammate.getNumBuilding(gc.getInfoTypeForString('BUILDING_EYES_AND_EARS_NETWORK')) > 0:
##															bCanSeeResearch = True
##															break
												...
												if CyGame().isVotePassed(gc.getInfoTypeForString('VOTE_COORDINATE_RESEARCH')):
													if gc.getPlayer(ePlayer).isFullMember(gc.getInfoTypeForString('DIPLOVOTE_OVERCOUNCIL')) and gc.getPlayer(gc.getGame().getActivePlayer()).isFullMember(gc.getInfoTypeForString('DIPLOVOTE_OVERCOUNCIL')):
															bCanSeeResearch = True
	...
	def updateManaStrings( self ):
		...
		if CyInterface().isScoresVisible() and not CyInterface().isCityScreenUp() and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY and CyEngine().isGlobeviewUp() == False and bShowManaBar == 1:
			for szBonus in lManaTypes:
				...
				iNumMana = pPlayer.getNumAvailableBonuses(iBonus)
				szBuffer = u"<font=2>"
				szTempBuffer = u"%c: %d" %(gc.getBonusInfo(iBonus).getChar(), iNumMana)
				szBuffer += szTempBuffer
				iSupplimental = cf.getNumSupplimentalManaPlayer(iPlayer, iBonus)
				if iSupplimental > 0:
					szBuffer += ' (+' + str(iSupplimental) + ')' #'='+ str(iNumMana+iSupplimental)
				elif iSupplimental < 0:
					szBuffer += ' (' + str(iSupplimental) + ')' # + '='+ str(iNumMana+iSupplimental)
				szBuffer += "</font>"
				if ( CyInterface().determineWidth( szBuffer ) > iWidth ):
					iWidth = CyInterface().determineWidth( szBuffer )
				szName = "ManaText" + szBonus
##				screen.setText( szName, "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, 35, yCoord + (iCount * iBtnHeight) + 24, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, iBonus, -1 )
				screen.setText( szName, "Background", szBuffer, CvUtil.FONT_LEFT_JUSTIFY, 0, yCoord + (iCount * iBtnHeight) + 24, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, iBonus, -1 )
				screen.show(szName)
				iCount = iCount + 1

			screen.setPanelSize( "ManaBackground", 6, yCoord + 18, (iWidth) + 15, (iBtnHeight * 21) + 12 )
