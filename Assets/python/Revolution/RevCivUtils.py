...
#Magister
			if iReligion != -1:
				...
				infoLeader = gc.getLeaderHeadInfo(iLeader)
				if infoLeader.hasTrait(iAgnostic) or infoLeader.getReligionWeightModifier(iReligion) <= -95:
#Magister
					liResult.remove( iLeader )
					liBadLeaders.append( iLeader )
					continue
...
class RevCivDefines :
	def __init__( self ) :
		...
		self.iAnagantios = CvUtil.findLeaderNum( 'LEADER_ANAGANTIOS' )
		...
		self.liMinorLeaders[self.iIllians] = [self.iDumannios, self.iRiuros, self.iAnagantios, self.iBraeden]#Magister, fix typo
...
#Magister
		...
		self.iHand = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_WHITE_HAND' )
		self.iMatronae = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_MATRONAE' )
		
		self.iUnblemished = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_UNBLEMISHED' )
		self.iBrotherhood = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_BROTHERHOOD_OF_WARDENS' )
		self.iLaeranCord = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_LAERAN_CORD' )
		self.iFoxmen = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_FOXMEN' )
		self.iDragon = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_CULT_OF_THE_DRAGON' )
		self.iStewards = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_STEWARDS_OF_INEQUITY' )
		self.iCoven = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_COVEN' )
		self.iAnointed = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_ANOINTED' )
		self.iMono = CvUtil.findInfoTypeNum( gc.getReligionInfo, gc.getNumReligionInfos(), 'RELIGION_CHILDREN_OF_THE_ONE' )
...
#Magister
		self.iGlacier = CvUtil.findInfoTypeNum( gc.getTerrainInfo, gc.getNumTerrainInfos(), 'TERRAIN_GLACIER' )
		self.iWaste = CvUtil.findInfoTypeNum( gc.getTerrainInfo, gc.getNumTerrainInfos(), 'TERRAIN_WASTELAND' )
#Magister
		...
		for iCiv in range( 0, gc.getNumCivilizationInfos() ) :
			...
			self.lpCivRules[iCiv].liBlockedReligions = [self.iHand, self.iUnblemished, self.iBrotherhood, self.iLaeranCord, self.iFoxmen, self.iDragon, self.iStewards, self.iCoven, self.iAnointed, self.iMono]#Magister
		...
		self.lpCivRules[self.iCalabim].liBlockedReligions.append(self.iEmpyrean)#Magister

		self.lpCivRules[self.iClan].iRace = self.iOrc
		self.lpCivRules[self.iClan].liBlockedReligions.append(self.iOrder)#Magister

		self.lpCivRules[self.iDoviello].liBlockedReligions.remove(self.iHand)#Magister
		self.lpCivRules[self.iDoviello].liBlockedReligions.append(self.iOrder)#Magister
		...
		self.lpCivRules[self.iDoviello].pCivTerrainPreference.afTerrainAffinity[self.iWaste] = 1.5#Magister

		self.lpCivRules[self.iElohim].liReligions = [self.iEmpyrean, self.iOrder, self.iFellowship]#Magister
		self.lpCivRules[self.iElohim].liBlockedReligions.append(self.iVeil)#Magister
		self.lpCivRules[self.iElohim].iRace = self.iHuman
		self.lpCivRules[self.iElohim].liGoodRaces = [self.iNomad, self.iWinterborn]

		self.lpCivRules[self.iGrigori].liBlockedReligions = [self.iOrder, self.iEmpyrean, self.iKilmorph, self.iFellowship, self.iOverlords, self.iEsus, self.iVeil, self.iDragon, self.iMono,self.iHand, self.iUnblemished, self.iBrotherhood, self.iLaeranCord, self.iFoxmen, self.iDragon, self.iStewards, self.iCoven, self.iAnointed, self.iMono]#Magister
		...
		self.lpCivRules[self.iIllians].liBlockedReligions = [self.iOrder, self.iEmpyrean, self.iKilmorph, self.iFellowship, self.iOverlords, self.iEsus, self.iVeil, self.iDragon, self.iUnblemished, self.iBrotherhood, self.iLaeranCord, self.iFoxmen, self.iDragon, self.iStewards, self.iCoven, self.iAnointed, self.iMono]#Magister
		...
		self.lpCivRules[self.iIllians].pCivTerrainPreference.afTerrainAffinity[self.iGlacier] = 2.5#Magister
		...
		self.lpCivRules[self.iIllians].liBlockedReligions.remove(self.iDragon)#Magister
		self.lpCivRules[self.iKuriotates].liReligions = [self.iOrder, self.iDragon]#Magister
		...
		self.lpCivRules[self.iSheaim].liReligions = [self.iVeil, self.iDragon]#Magister
		self.lpCivRules[self.iSheaim].liBlockedReligions = [self.iOrder, self.iHand, self.iMono]#Magister
		self.lpCivRules[self.iSheaim].iRace = None

		self.lpCivRules[self.iSidar].liReligions = [self.iEsus, self.iFellowship]#Magister
