...
#Magister
			infoLeader = gc.getLeaderHeadInfo(pRevPlayer.getLeaderType())
			if infoLeader.getReligionWeightModifier(giveRelType) <= -95:
				giveRelType = -1
#Magister
			if( giveRelType >= 0 ) :
...
#				pRevPlayer.setLastStateReligion( giveRelType )
				...
				availRels = list()
				for relType in range(0,gc.getNumReligionInfos()) :
					if infoLeader.getReligionWeightModifier(relType) > -95:#Magister
