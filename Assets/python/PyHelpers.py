...
#Magister Start
	...
	def getCityPlotList(self):
		plotList = []
		for pCity in self.getCityList():
			plotList.append(pCity.plot())
		return plotList

	def getActAsCityPlotList(self):
		plotList = self.getCityPlotList()
		if gc.getGame().isOption(GameOptionTypes.GAMEOPTION_ADVANCED_TACTICS):
			lSuperForts = []
			for iImprovement in range (gc.getNumImprovementInfos()):
				if gc.getImprovementInfo(iImprovement).isActsAsCity():
					if self.player.getImprovementCount(iImprovement) > 0:
						lSuperForts.append(iImprovement)
			if len(lSuperForts) > 0:
				for pPlot in self.getPlotList():
					if not pPlot.isWater():
						if pPlot.getImprovementType() in lSuperForts:
...
# Players Plots
	...
	def getPlotsWithImprovement(self, iImprovement):
		' plotList with iBonus '
		lPlots = []
		for plot in self.getPlotList():
			if ( not plot.getImprovementType() == ImprovementTypes.NO_IMPROVEMENT and plot.getImprovementType() == iImprovement):
				lPlots.append(plot)
		return lPlots
...
#Magister Start
	def countNumBuildingGlobal(self,iBuilding):
		"int - Number of iBuildingIDs the whole map has"
		iCount = 0

		if CyGame().getBuildingClassCreatedCount(gc.getBuildingInfo(iBuilding).getBuildingClassType()) > 0:
			for i in range(gc.getMAX_CIV_PLAYERS()):
				iCount += gc.getPlayer(i).countNumBuildings(iBuilding)
		return iCount

	def doesBuildingExist(self,iBuilding):
		"Bool - Is there a iBuildingIDs in the world"
		if CyGame().getBuildingClassCreatedCount(gc.getBuildingInfo(iBuilding).getBuildingClassType()) > 0:
			for i in range(gc.getMAX_CIV_PLAYERS()):
				if gc.getPlayer(i).countNumBuildings(iBuilding) > 0:
					return True
		return False

	def getUnitClassCountGlobal(self,iUnitClass):
		"int - Number of iUnitClassIDs the whole map has"
		iCount = 0
		for i in range(gc.getMAX_CIV_PLAYERS()):
			iCount += gc.getPlayer(i).getUnitClassCount(iUnitClass)
		return iCount

	def doesUnitClassExist(self,iUnitClass):
		"Bool - Is there a iUnitClassIDs in the world"
		for i in range(gc.getMAX_CIV_PLAYERS()):
			if gc.getPlayer(i).getUnitClassCount(iUnitClass) > 0:
				return True
		return False


	def getUnitClassCountGlobal(self,iUnit):
		"int - Number of iUnitIDs the whole map has"
		iCount = 0
		for i in range(gc.getMAX_CIV_PLAYERS()):
			iCount += gc.getPlayer(i).getUnitClassCount(iBuilding)
		return iCount
