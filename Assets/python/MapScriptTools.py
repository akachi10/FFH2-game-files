...
#	print "[MST] " + sVersion
	bBTS = (sVersion.find("Beyond the Sword") > -1) or (sVersion.find(u"\u5200\u5251") > -1)	# YK-L10N: match Chinese BtS text (DaoJian) too
...
#	print "[MST] " + sVersion
	...
	if bRifE:
		modName = sVersion.encode('latin_1', 'replace')	# YK-L10N: str() dies on CJK version text
...
def mapSetSign( plot, sTxt, noSign=False, iPlayerNum=None ):
	signText = sTxt	# YK-L10N: engine sign API needs unicode for CJK
	log = ""
	if iPlayerNum == None:
		...
		log += " Player %i:" % (player)
	log += " Plot %i,%i named: %s \n" % ( plot.getX(), plot.getY(), CvUtil.convertToStr(sTxt) )
	return log
