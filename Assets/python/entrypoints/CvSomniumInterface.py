...
#Magister- cut times in half, as it makes the game too slow
		# times for turn movie
		# HU pauses
		self.T_0 = 0.25 #pause after drawing the death -> update info
		self.T_1 = 1 #pause after update info after drawing the death -> update center
		self.T_2 = 0.75 #pause after info draw jok -> discard if no card to take
		self.T_3 = 1 #pause after drawing double type ,update info panel -> update center
		self.T_4 = 1 #pause after info panel when no more card to draw -> reset center
		self.T_5 = 1 #pause after bank cards -> and reset center cards
		self.T_6 = 0.75 #pause after stealing a card with a jok -> updates (except direct player following)
		self.T_7 = 1 #pause after stealing a card with a jok double type -> update center
		self.T_8 = 1 #pause after stealing a card with a jok -> no more card
		# AI pauses
		self.T_9 = 0.75 #pause info panel begin AI -> turn begin
		self.T_10 = 0.75 #pause after AI draw
		self.T_11 = 1 #pause after AI draw death -> update center
		self.T_12 = 0.75 #pause after AI joker steal
		self.T_13 = 0.75 #pause after AI joker no use
		self.T_14 = 1 #pause after AI draw same type -> update center
		self.T_15 = 1 #pause after AI bank -> update center
		# end game
		self.T_16 = 2 #pause before closing the screen , and end game
	...
	def getAggressiveness(self, iPlayer):
		...
		iAggressiveness = gc.getLeaderHeadInfo(gc.getPlayer(self.playerRight[iPlayer]).getLeaderType()).getSomniumAggressiveness()
		iAggressiveness = min(100, max(0, iAggressiveness))
